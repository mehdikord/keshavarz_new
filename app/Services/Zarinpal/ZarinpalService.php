...
class ZarinpalService
{
    ...
    {
        $this->merchant = config('gateway.zarinpal_merchant');
        $this->sandbox = config('gateway.zarinpal_sandbox');
        $this->zaringate = config('gateway.zarinpal_gate');
    ...
    public function request($Amount, $OrderId='',$CallbackURL,$Mobile="", $Email="",$Description="")
    {
        ...
        {
            $node 	= $SandBox ? "sandbox" : $this->zarinpal_node();
            ...
            $StartPayUrl 	= (isset($ZarinGate) 			&& $ZarinGate == true) 			? "{$StartPay}/ZarinGate" : $StartPay.'/Sad';
    ...
    {
        $SandBox = $this->sandbox;
        $ZarinGate = $SandBox ? false : $this->zaringate;
        ...
        if ($this->soap_check() === true)
        {
            ...
            $data = array('MerchantID' => $this->merchant, 'Authority' => $au, 'Amount' => $Amount);
            $jsonData = json_encode($data);
