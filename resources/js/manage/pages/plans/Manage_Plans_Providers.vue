<template>
    <q-card flat bordered>
        ...
        <q-card-section class="bg-blue-grey-14 text-white q-pb-lg">
            ...
            <strong class="font-16">لیست همه اشتراک های خدمات دهنده</strong>
        </q-card-section>
...
export default {
    ...
    methods:{
        ...mapActions([
            ...
            "PlansProviderStore",
            "PlansProviderDelete",
            "PlansProviderDeleteImage",
            "PlansProviderEdit",
            "PlansProviderEditImage",
            "PlanProviderChangeActive"

        ]),
        GetItems(){
            this.PlansProviderIndex().then(res => {
        ...
        AddItem(){
            ...
            this.PlansProviderStore(this.add).then(res => {
        ...
        EditItem(item){
            this.loading_add=true;
            this.PlansProviderEdit(item).then(res => {
                this.loading_add=false;
        ...
        EditImage(id){
            ...
            this.PlansProviderEditImage({id:id,image:this.edit_image[id]}).then(res => {
                this.items = this.items.filter(item_get =>{
                    if (item_get.id === id){
        ...
        DeleteItem (id) {
            this.$q.dialog({
                ...
                this.PlansProviderDelete(id).then(res => {
        ...
        DeleteItemImage (id) {
            this.PlansProviderDeleteImage(id).then( res => {
                this.items = this.items.filter(item_get =>{
                    if (item_get.id === id){
                        item_get.image=null
                    }
        ...
        ChangeStatus(id){
            this.PlanProviderChangeActive(id).then(res => {
